...
export default function MapSection({osrmRoute, onStopClick}: MapProps) {
  ...
  return (
    <Box className="flex-1 relative bg-slate-100">
      <MapContainer center={[49.7437, 13.3736]} zoom={13} className="w-full h-full" zoomControl={false}>
        <BasemapTileLayer/>

        <NetworkStops onStopClick={onStopClick} selectedStopId={null} filter={true}/>

        {osrmRoute.length > 1 && (
          <Polyline positions={osrmRoute} pathOptions={{color: typeColor, weight: 5, opacity: 0.8}}/>
        )}

        <RouteWaypoints typeColor={typeColor}/>
      </MapContainer>
